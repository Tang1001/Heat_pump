...
T5_initial=T_buttom_water_tank2_130423(421); % layer 5 initial temperature


d_T1 = (T1_initial - T3_initial) * (R1 / R1_tot);
T2_initial = T1_initial - d_T1;     % layer 2 initial temperature
d_T3 = (T3_initial - T5_initial) * (R3 / R2_tot);
...
%% Euler method
T_results = system_of_equations_Euler_5layers_4R(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3, m4, m5, cp, R12, R23, R34, R45, m_c_dot, diff_T_c, T_s);
...
objective_function = @(params) objFun_Euler_5layers_R1R2R3(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3, m4, m5, cp, m_c_dot, diff_T_c, T_s, params, data);

% Call fminunc
lb = [0, 0, 0];
ub = [50,50,50];
options = optimoptions('fmincon','Display','iter','Algorithm','sqp');
R_optimal = fmincon(objective_function, R0, [], [], [], [], lb, ub, [], options)

R12_optimal = R_optimal(1);
R34_optimal = R_optimal(2);
R45_optimal = R_optimal(2);
%%
T_results = system_of_equations_Euler_5layers(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3, m4, m5, cp, R12_optimal, R34_optimal, R45_optimal, m_c_dot, diff_T_c, T_s);


%% R12, R34, R45    m3, m4, m5
% Initial guess for R1 and R2
R0 = [R12, R34, R45];
M0 = [m3,m4,m5];
P0 = [R0,M0];
...
% Define the objective function
objective_function = @(params) objFun_Euler_5layers_R1R2R3m3m4m5(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, cp, m_c_dot, diff_T_c, T_s, params, data);

% Call fminunc
Aeq = [0,0,0,1,1,1];
beq = 500;
lb = [0, 0, 0, 0, 0, 0];
ub = [1,1,1, 500, 500, 500];
options = optimoptions('fmincon','Display','iter','Algorithm','sqp');
R_optimal = fmincon(objective_function, P0, [], [], Aeq, beq, lb, ub, [], options)

R12_optimal = R_optimal(1);
R34_optimal = R_optimal(2);
R45_optimal = R_optimal(3);
m3_optimal = R_optimal(4);
m4_optimal = R_optimal(5);
m5_optimal = R_optimal(6);

%%
T_results = system_of_equations_Euler_5layers(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3_optimal, m4_optimal, m5_optimal, cp, R12_optimal, R34_optimal, R45_optimal, m_c_dot, diff_T_c, T_s);



%% R12, R34, R45    m3, m4, m5  diff_T_c
% Initial guess for R1 and R2
R0 = [R12, R34, R45];
M0 = [m3,m4,m5];
P0 = [R0,M0,diff_T_c];
...
% Define the objective function
objective_function = @(params) objFun_Euler_5layers_R1R2R3m3m4m5_diffT(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, cp, m_c_dot, T_s, params, data);

% Call fminunc
Aeq = [0,0,0,1,1,1,0];
beq = 500;
lb = [0, 0, 0, 0, 0, 0, 0];
ub = [1,1,1, 500, 500, 500, 5];
options = optimoptions('fmincon','Display','iter','Algorithm','sqp');
R_optimal = fmincon(objective_function, P0, [], [], Aeq, beq, lb, ub, [], options)
...
m4_optimal = R_optimal(5);
m5_optimal = R_optimal(6);
diff_T_c_optimal = R_optimal(7);

%%
T_results = system_of_equations_Euler_5layers(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3_optimal, m4_optimal, m5_optimal, cp, R12_optimal, R34_optimal, R45_optimal, m_c_dot, diff_T_c_optimal, T_s);
...
% Define the objective function
objective_function = @(params) objFun_Euler_5layers_4R_3m_diffT(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, cp, m_c_dot, T_s, params, data);

% Call fminunc
Aeq = [0,0,0,0,1,1,1,0];
beq = 500;
lb = [0, 0, 0, 0, 0, 0, 0, 0];
ub = [1,0.1,1,1, 500, 500, 500, 5];
options = optimoptions('fmincon','Display','iter','Algorithm','sqp');
R_optimal = fmincon(objective_function, P0, [], [], Aeq, beq, lb, ub, [], options)
...
m3_optimal = R_optimal(5);
m4_optimal = R_optimal(6);
m5_optimal = R_optimal(7);
diff_T_c_optimal = R_optimal(8);

%%
T_results = system_of_equations_Euler_5layers_4R(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3_optimal, m4_optimal, m5_optimal, cp, R12_optimal, R23_optimal, R34_optimal, R45_optimal, m_c_dot, diff_T_c_optimal, T_s);
...
%%
T_results = system_of_equations_Euler_5layers(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp,  m1, m2, m3_optimal, m4_optimal, m5_optimal, cp, R12, R34, R45, m_c_dot, diff_T_c, T_s);
