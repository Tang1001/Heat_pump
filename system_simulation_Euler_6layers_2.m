...
T_upper_water_tank2_interp = interp1(time_data_8min,T_upper_water_tank2(round(60/8*7):round(60/8*19)+1),time_data);
...
T_results = system_of_equations_Euler_6layers_5R_singlePipe(time_data, T, m_p_dot_interp, T_in_interp, m_s_dot_interp, m1, m2, m3_optimal, m4_optimal, m5_optimal, m6_optimal, cp, R12, R23_optimal, R34_optimal, R45_optimal, R56_optimal, m_c_dot_optimal, diff_T_c_optimal, T_s_optimal);
...
err2 = sqrt(mean((T_results(:,3) - data(:,2)).^2));               %Root Mean Squared Error
err3 = sqrt(mean((T_results(:,6) - data(:,3)).^2));               %Root Mean Squared Error
figure;
subplot(3,1,1)
...
grid on
subplot(3,1,2)
plot(time_data, T_results(:,3),time_data_8min, T_upper_water_tank2((15*3+8):(9*15+8)+1));
legend("layer 3","upper water of tank 2")
xlabel("Time (0=07:00; 12=19:00)")
ylabel("Temp (°C)")
xlim([0 12])
