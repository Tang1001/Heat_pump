...
% %% 1R   m_layer_tank1 m_layer_tank2  diff_T_c T_s m_c_dot
% R0 = [R_tank12];
% % M0 = [m3,m4,m5,m6];
% P0 = [R0,m_layer_tank1',m_layer_tank2',diff_T_c,T_s,m_c_dot];
% 
% 
% % Define the objective function
% objective_function = @(params) objFun_Euler_NandNlayers(time_data, T_initial, m_p_dot_interp, T_in_interp, m_s_dot_interp,num_layer_tank1,num_layer_tank2, params, T_ground_truth);
% % Call fminunc
% Aeq = [0, ones(1,num_layer_tank1), zeros(1,num_layer_tank2), 0,0,0;
%     0, zeros(1,num_layer_tank1), ones(1,num_layer_tank2), 0,0,0];
% beq = [500;500];
% lb = [0, zeros(1,num_layer_tank1+num_layer_tank2), 1,9,800];
% ub = [0.1, ones(1,num_layer_tank1+num_layer_tank2)*500, 5,15,1200];
% options = optimoptions('fmincon','Display','iter','Algorithm','sqp');
% options.MaxFunctionEvaluations = 3000;
% P_optimal = fmincon(objective_function, P0, [], [], Aeq, beq, lb, ub, [], options);
% 
% 
% R_tank12_optimal = P_optimal(1);
% m_layer_tank1_optimal=zeros(num_layer_tank1,1);
% m_layer_tank2_optimal=zeros(num_layer_tank2,1);
% for i=1:num_layer_tank1
%     m_layer_tank1_optimal(i) = P_optimal(i+1);
% end
% for i=1:num_layer_tank2
%     m_layer_tank2_optimal(i) = P_optimal(i+1+num_layer_tank1);
% end
% 
% diff_T_c_optimal = P_optimal((1+num_layer_tank1+num_layer_tank2)+1)
% T_s_optimal = P_optimal((1+num_layer_tank1+num_layer_tank2)+2)
% m_c_dot_optimal = P_optimal((1+num_layer_tank1+num_layer_tank2)+3)
% 
% 
% % m_layer_tank2=[m3_optimal;m4_optimal;m5_optimal;m6_optimal];
% T_results = system_of_equations_Euler_Nlayers(time_data, T_initial, m_p_dot_interp, T_in_interp, m_s_dot_interp, num_layer_tank1,num_layer_tank2, m_layer_tank1_optimal, m_layer_tank2_optimal, R_tank12_optimal, m_c_dot_optimal, diff_T_c_optimal, T_s_optimal);
% 
% 
% % Plot Figure
