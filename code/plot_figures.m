...
err1 = sqrt(mean((T_results(:,1) - ground_truth(:,1)).^2));               %Root Mean Squared Error
err2 = sqrt(mean((T_results(:,num_layer_tank1+1) - ground_truth(:,2)).^2));               %Root Mean Squared Error
err3 = sqrt(mean((T_results(:,num_layer_tank1+num_layer_tank2) - ground_truth(:,3)).^2));               %Root Mean Squared Error
figure;
subplot(3,1,1)
...
grid on
subplot(3,1,3)
plot(time_data, T_results(:,num_layer_tank1+num_layer_tank2),time_data_1min, T_buttom_tank2_data);
xlabel("Time (0=07:00, 12=19:00)")
ylabel("Temp (°C)")
