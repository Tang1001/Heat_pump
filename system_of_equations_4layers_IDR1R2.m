function dTdt = system_of_equations_4layers_IDR1R2(t, T, m_p_dot_data, T_in_data, m_s_dot_data, time_data_1min, time_data_1h, m1, m2, m3, m4, cp, m_c_dot, diff_T_c, T_s, params)
    T1 = T(1);
    T2 = T(2);
    T3 = T(3);
    T4 = T(4);
    ...
    dT2dt = (m_p_dot * cp * (T1 - T2) + R1 * (T1 - T2)*(60*60) + m_s_dot * cp * (T3 - T2)) / (m2 * cp);
    dT3dt = (m_p_dot * cp * (T2 - T3) - R2 * (T3 - T4)*(60*60) + m_s_dot * cp * (T4 - T3)) / (m3 * cp);
