function T_results = system_of_equations_Euler_4layers_R1R2R3(time_data, T_initial, m_p_dot_data, T_in_data, m_s_dot_data, m1, m2, m3, m4, cp, R1, R2, R3, m_c_dot, diff_T_c, T_s)
...
for k = 1:length(time_data) - 1
    ...
    dT2dt = (m_p_dot * cp * (T(1) - T(2)) + R1 * (T(1) - T(2))*(60*60) - R3 * (T(2) - T(1))*(60*60) + m_s_dot * cp * (T(3) - T(2))) / (m2 * cp);
    dT3dt = (m_p_dot * cp * (T(2) - T(3)) - R2 * (T(3) - T(4))*(60*60) + R3 * (T(2) - T(1))*(60*60) + m_s_dot * cp * (T(4) - T(3))) / (m3 * cp);
