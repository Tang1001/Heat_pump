...
% % Time span for the simulation
% t_start = 0+9/60;
% t_end = 44/60;     % end time [h]
% sample_time = 2;    % [min]
% time_span = [t_start:sample_time/60:t_end];
% time_data = time_span;
% 
% %1min
% m_p_dot_data=FR_hp(1+9:1+44)*1000;  %[L/h] 60*7+1+5~60*(7+11.5)+1
% T_in_data=T_out_he(1+9:1+44);
% T_upper_tank1_data = T_upper_tank1(1+9:1+44);
% T_bottom_tank2_data = T_bottom_tank2(1+9:1+44);
% 
% %8min
% T_upper_tank2_data = T_upper_tank2(1+1:6+1); %0:08~0:48
% 
% %1h
% m_s_dot_data=V_water(1:2)*1000;    %[L/h] 0:00~1:00
% 
% time_data_1min=9/60:1/60:44/60;
% time_data_1h=floor(time_data(1)):ceil(time_data(end));
% time_data_8min=8/60:8/60:48/60;

%17:02 ~ 18:33
%17+2/60~18+33/60
% Time span for the simulation
t_start = 17+2/60;
t_end = 18+33/60;     % end time [h]
sample_time = 2;    % [min]
time_span = [t_start:sample_time/60:t_end];
time_data = time_span;

%1min
m_p_dot_data=FR_hp(1+60*17+2:1+60*18+33)*1000;  %[L/h] 60*7+1+5~60*(7+11.5)+1
T_in_data=T_out_he(1+60*17+2:1+60*18+33);
T_upper_tank1_data = T_upper_tank1(1+60*17+2:1+60*18+33);
T_bottom_tank2_data = T_bottom_tank2(1+60*17+2:1+60*18+33);

%8min
T_upper_tank2_data = T_upper_tank2(1+128:1+139); %17:04~18:32

%1h
m_s_dot_data=V_water(1+17:1+19)*1000;    %[L/h] 0:00~1:00
...
T_outdoor = 15*ones(length(time_data),1);
...
plot_comparison_T_heatloss(time_data,T_results,time_data_1min,time_data_8min,num_layer_tank1,num_layer_tank2,T_upper_tank1_data,T_upper_tank2_data,T_bottom_tank2_data)
